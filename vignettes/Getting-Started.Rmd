...
We also need to specify initial values for the parameters. These are passed as a vector to the `init` argument, with the corresponding elements of the vector matching the order of the parameters in the function. This is currently required for determining the total number of parameters to estimate, and will be removed in a future version of `StanEstimators`. For now we will just specify all initial values as 1:
...
We can now pass these arguments to the `stan_sample` function to estimate our model. We will use the default number of chains (4) and the default number of warmup iterations (1000) and sampling iterations (1000).
...
The estimates are stored in the `fit` object using the [`posterior::draws` format](https://cran.r-project.org/web/packages/posterior/vignettes/posterior.html). We can inspect the estimates using the `summary` function (which calls `posterior::summarise_draws`):
