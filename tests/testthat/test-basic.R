...
test_that("stan_optimize runs", {
  ...
  expect_no_error(
    opt_gd <- stan_optimize(loglik_fun, inits, additional_args = list(y), grad_fun = grad,
                        lower = c(-Inf, 0), seed = 1234)
  )
  expect_no_error(
    opt_gd_bfgs <- stan_optimize(loglik_fun, inits, additional_args = list(y), grad_fun = grad,
                        lower = c(-Inf, 0),
                        algorithm = "bfgs",
                        seed = 1234)
  )
...
test_that("stan_variational runs", {
  expect_no_error(
    var_fd <- stan_variational(loglik_fun, inits, additional_args = list(y), lower = c(-Inf, 0),
                         seed = 1234)
  )
  expect_no_error(
    var_gd <- stan_variational(loglik_fun, inits, additional_args = list(y), grad_fun = grad,
                        lower = c(-Inf, 0), seed = 1234)
  )
  expect_no_error(
    var_gd_fullrank <- stan_variational(loglik_fun, inits, additional_args = list(y), grad_fun = grad,
                        lower = c(-Inf, 0),
                        algorithm = "fullrank",
                        seed = 1234)
  )
...
test_that("stan_pathfinder runs", {
  expect_no_error(
    path_fd <- stan_pathfinder(loglik_fun, inits, additional_args = list(y), lower = c(-Inf, 0),
                         seed = 1234)
  )
  expect_no_error(
...
test_that("stan_laplace runs", {
  opt_fd <- stan_optimize(loglik_fun, inits, additional_args = list(y), lower = c(-Inf, 0),
                        seed = 1234)
  opt_mode <- stan_laplace(loglik_fun, inits, additional_args = list(y),
                            lower = c(-Inf, 0),
                            mode = opt_fd)
  num_mode <- stan_laplace(loglik_fun, inits, additional_args = list(y),
                            lower = c(-Inf, 0),
                            mode = c(10, 2))
  no_mode <- stan_laplace(loglik_fun, inits, additional_args = list(y),
                            lower = c(-Inf, 0))
})
