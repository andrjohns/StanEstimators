...
RcppExport SEXP call_stan_(SEXP options_vector, SEXP ll_fun, SEXP grad_fun) {
  ...
  const int n = Rf_xlength(options_vector);
  std::vector<const char*> char_opts(n);

  for (int i = 0; i < n; i++) {
    char_opts[i] = Rf_translateCharUTF8(STRING_ELT(options_vector, i));
  }

  return Rcpp::wrap(cmdstan::command(char_opts.size(), char_opts.data()));
  END_RCPP
}
